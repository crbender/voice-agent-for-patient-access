...
Direct scheduling requests without a valid capability, with an expired capability, or with a capability from another session return `validation_required`; the browser verification flag is only a UX guard. Session and capability state is in-memory and disappears when the local server restarts.
