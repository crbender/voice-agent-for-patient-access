...
For rescheduling, live voice mode exposes a local Realtime tool named `confirm_appointment_reschedule`. Caller transcription is synchronized to the local server, which validates both factors against its canonical active demo profile and issues a three-minute opaque scheduling capability bound to that Realtime session. When Riley has captured a requested appointment window, the browser bridges the model tool call to `/api/demo-tools/confirm-appointment` with that capability, then sends the deterministic result back to Realtime as `function_call_output` for the same tool-call ID.

Direct scheduling requests without a valid capability, with an expired capability, or with a capability from another session return `validation_required`; the browser verification flag is only a UX guard. Capability expiry also clears accumulated verification evidence, so both factors must be provided again before the server can issue a replacement. Session and capability state is in-memory and disappears when the local server restarts.
