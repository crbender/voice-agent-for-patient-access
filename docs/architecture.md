...
- Caller transcription is validated against the server-owned active demo profile; only then does the server issue a short-lived opaque scheduling capability bound to that Realtime session
...
- maintains bounded, expiring in-memory demo session/verification state and rejects scheduling calls without the matching capability
...
The patient-access scenario exposes `confirm_appointment_reschedule` as a Realtime tool. Before tool use, the browser forwards caller transcription to the local verification endpoint; the server accumulates bounded evidence for the opaque local Realtime session and issues a three-minute scheduling capability only after both canonical active-profile factors match. When that capability expires, the server also clears the verified flag and accumulated evidence, so both factors must be provided again before renewal. The browser receives the model's function call over the Realtime data channel, calls the local Python stub with that session-bound capability, then returns the tool output to the model as `function_call_output` using the same tool-call ID. The stub waits briefly so the demo visibly feels like an external scheduling lookup, then deterministically returns ranked canonical slots for broad requests or a mock confirmation for an exact slot/ID match. Missing, invalid, expired, or cross-session capabilities return `validation_required`; negated, contradictory, ambiguous, or mismatched requests return clarification rather than a success-shaped result.
